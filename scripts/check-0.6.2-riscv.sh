...
KERNEL="$ROOT/kernel-console-v062.elf"
LOG="$(mktemp /tmp/agentos-062rv-check.XXXXXX)"
FIFO_IN="$(mktemp -u /tmp/agentos-062rv-stdin.XXXXXX).fifo"

cd "$ROOT"
make kernel-console-v062.elf
mkfifo "$FIFO_IN"
...
wait_console_qemu "$LOG" "$QPID" "0.6.2 demo complete" 480
stop_console_qemu "$QPID" "$WRITER"

for pat in "OpenAgentOS 0.6.2" "\\[fleet\\] ingest net ok" "answer: 42" "0.6.2 demo complete"; do
  rg -q "$pat" "$LOG" || { echo "[check-0.6.2-riscv] missing $pat"; tail -100 "$LOG"; exit 1; }
done
rm -f "$LOG"
echo "[check-0.6.2-riscv] ok"
