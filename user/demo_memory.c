...
static void memory_worker(void) {
    char version[64];
    ...
    agent_log_str("[memory] reading /sys/version via storage\n");
    if (agent_svc_read("/sys/version", version, sizeof(version)) < 0) {
        agent_log_str("[memory] read /sys/version failed\n");
        agent_exit(1);
    }
    agent_log_str("[memory] version: ");
    agent_log_str(version);
    agent_log_str("\n");

    if (agent_session_tail(session_line, sizeof(session_line)) >= 0 &&
        session_line[0]) {
        agent_log_str("[memory] restored prior session tail: ");
        agent_log_str(session_line);
        agent_log_str("\n");
    }

    prompt[0] = '\0';
    str_cat(prompt, "AgentOS version: ", sizeof(prompt));
    str_cat(prompt, version, sizeof(prompt));
    str_cat(prompt, ". What is 17+25? Reply with only the number.",
            sizeof(prompt));

    if (agent_session_append("user: memory-query") < 0) {
        agent_log_str("[memory] session append user failed\n");
        agent_exit(1);
    }

    agent_log_str("[memory] querying LLM...\n");
    n = agent_llm(prompt, response, sizeof(response));
    if (n < 0) {
        agent_log_str("[memory] llm failed err=");
        agent_log_int(n);
        agent_log_str("\n");
        agent_exit(1);
    }
    agent_log_str("[memory] llm answer: ");
    agent_log_str(response);
    agent_log_str("\n");

    if (agent_sync() < 0) {
        agent_log_str("[memory] sync failed\n");
        agent_exit(1);
    }
    agent_log_str("[memory] persist sync ok\n");

    if (agent_session_tail(session_line, sizeof(session_line)) < 0) {
        agent_log_str("[memory] session tail failed\n");
        agent_exit(1);
    }
    agent_log_str("[memory] session tail: ");
    agent_log_str(session_line);
    agent_log_str("\n");

    if (!str_starts_with(session_line, "assistant:")) {
        agent_log_str("[memory] unexpected session tail\n");
        agent_exit(1);
    }
...
void init_memory_agent(void) {
    agent_log_str("[init] v1.2 memory + LLM + persist demo\n");

    int wid = agent_spawn(memory_worker, "memory-worker",
