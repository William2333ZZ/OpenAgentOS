...
#ifndef INIT_AGENT
#define INIT_AGENT init_net_prod_agent
#endif

extern void network_prod_agent_main(void);

void kmain(unsigned long hartid, unsigned long dtb) {
    smp_boot(hartid, dtb);

    agentos_boot_banner("v4.3 Production network demo");
    agentos_trap_init();
    mem_init();
    vm_init();
    ramfs_init();
    ...
    net->id = NET_AGENT_ID;
    agent_start_service(net, network_prod_agent_main, "network",
                        CAP_LOG | CAP_SVC_NET | CAP_NET | CAP_SEND | CAP_RECV | CAP_FS);
    ...
    kprintf("[kernel] prod network id=%d (virtio-net native TCP/HTTP)\n", NET_AGENT_ID);
