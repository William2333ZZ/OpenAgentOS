...
extern void init_v060_agent(void);

void kmain(void) {
    smp_boot(0, 0);

    agentos_boot_banner("0.6.0 — agent sandbox-net (x86 parity)");
    agentos_trap_init();
    mem_init();
    vm_init();
    ramfs_init();
    ...
    netstack_init();
    if (!netstack_ready())
        kprintf("[kernel] net: virtio-net-pci not ready — fleet/remote tcp disabled\n");
    tenant_init();
    audit_init();
    namespace_init();
    quota_init();
