...
extern void init_v061_agent(void);

void kmain(unsigned long hartid, unsigned long dtb) {
    smp_boot(hartid, dtb);

    agentos_boot_banner("0.6.1 — virtio-net + agent sandbox-net + HTTPS fleet");
    agentos_trap_init();
    mem_init();
    vm_init();
    ramfs_init();
