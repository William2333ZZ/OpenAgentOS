...
extern void init_v050_agent(void);

void kmain(unsigned long hartid, unsigned long dtb) {
    smp_boot(hartid, dtb);

    agentos_boot_banner("0.5.0 — virtio-net + DeepSeek console");
    agentos_trap_init();
    mem_init();
    vm_init();
    ramfs_init();
