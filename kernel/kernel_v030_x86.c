...
extern void init_v030_agent(void);

void kmain(void) {
    smp_boot(0, 0);

    agentos_boot_banner("0.3.0 — fleet ingest + remote ping");
    agentos_trap_init();
    mem_init();
    vm_init();
    ramfs_init();
