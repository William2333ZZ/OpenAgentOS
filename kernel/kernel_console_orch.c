...
#ifndef INIT_AGENT
#define INIT_AGENT init_console_orch_agent
#endif
...
void kmain(unsigned long hartid, unsigned long dtb) {
    ...
    agentos_boot_banner("v5.2 Console Orchestrator demo");
    agentos_trap_init();
    mem_init();
    vm_init();
    ramfs_init();
    ...
    agent_start_service(storage, storage_agent_main, "storage",
                        CAP_LOG | CAP_SVC_STORAGE | CAP_FS | CAP_SEND | CAP_RECV);
    ...
    kprintf("[kernel] storage id=%d console id=%d\n", STORAGE_AGENT_ID, CONSOLE_AGENT_ID);
