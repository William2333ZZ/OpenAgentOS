...
#ifndef INIT_AGENT
#define INIT_AGENT init_console_namespace_agent
#endif
...
void kmain(unsigned long hartid, unsigned long dtb) {
    ...
    agentos_boot_banner("v6.2 Namespace mount demo");
    agentos_trap_init();
    mem_init();
    vm_init();
    ramfs_init();
    ...
    seed.id = 1;
    namespace_write_secret(&seed, "agent1-home-secret", 18);
    ...
    kprintf("[kernel] console id=%d (namespace mount)\n", CONSOLE_AGENT_ID);
