...
extern void init_v063_agent(void);

void kmain(void) {
    smp_boot(0, 0);

    agentos_boot_banner("0.6.3 — agent sandbox-net (x86 parity)");
    agentos_trap_init();
    mem_init();
    vm_init();
    ramfs_init();
