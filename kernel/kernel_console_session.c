...
#ifndef INIT_AGENT
#define INIT_AGENT init_console_session_agent
#endif
...
void kmain(unsigned long hartid, unsigned long dtb) {
    ...
    agentos_boot_banner("v5.1 Console Session demo");
    agentos_trap_init();
    mem_init();
    vm_init();
    ramfs_init();
    ...
    kprintf("[kernel] router id=%d display id=%d console id=%d\n",
            ROUTER_AGENT_ID, DISPLAY_AGENT_ID, CONSOLE_AGENT_ID);
